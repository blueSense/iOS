...
- (IBAction)sliderValueChanged:(UISlider *)sender
{
    ...
    {
        self.beacon.configuration.signalStrength = (int) val;
        self.signalStrength.text = [NSString stringWithFormat:@"%ld", (long) self.beacon.configuration.signalStrength];
    }

    if (sender == self.advIntervalSlider)
    {
        self.beacon.configuration.advertisementInterval = val * 16;
        self.advInterval.text = [NSString stringWithFormat:@"%d", (int) (self.beacon.configuration.advertisementInterval / 1.6)];
    }
...
- (void)configureView
{
    ...
    if (self.beacon)
    {
        ...
        self.description.text = self.beacon.serial;

        self.uuid.text = self.beacon.configuration.uuid;
        self.uuid.delegate = self;

        self.major.text = [NSString stringWithFormat:@"%d", (int) self.beacon.configuration.major];
        self.major.delegate = self;

        self.minor.text = [NSString stringWithFormat:@"%d", (int) self.beacon.configuration.minor];
        self.minor.delegate = self;

        self.advInterval.text = [NSString stringWithFormat:@"%d", (int) (self.beacon.configuration.advertisementInterval / 1.6)];
        self.advIntervalSlider.value = self.beacon.configuration.advertisementInterval / 16.0;

        self.signalStrength.text = [NSString stringWithFormat:@"%d", (int) self.beacon.configuration.signalStrength];
        self.signalStrengthSlider.value = self.beacon.configuration.signalStrength;
    }
}
...
- (IBAction)update:(id)sender
{
    ...
    self.beacon.configuration.major = major;
    self.beacon.configuration.minor = minor;
...
- (void)refreshStatus
{
    ...
    allMatch = [self displayMismatch:self.major isMatch:(self.beacon.configuration.major == self.beacon.service.major)] && allMatch;
    allMatch = [self displayMismatch:self.minor isMatch:(self.beacon.configuration.minor == self.beacon.service.minor)] && allMatch;
    allMatch = [self displayMismatch:self.advInterval isMatch:(self.beacon.configuration.advertisementInterval == self.beacon.service.advertisementInterval)] && allMatch;
    allMatch = [self displayMismatch:self.signalStrength isMatch:(self.beacon.configuration.signalStrength == self.beacon.service.signalStrength)] && allMatch;
